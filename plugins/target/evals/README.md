# Target agentic evals

Four Harbor tasks cover store-scoped product search, TCIN lookup, nearby pickup
availability, and shopping-list pricing. Each task runs the MCP server and a deterministic local
stand-in for Target's three public Redsky website endpoints inside one task-local
...
review. Uploaded runs are named `ci-evals-target`.
