# Trader Joe's agentic evals

Four Harbor tasks cover store discovery, product search, SKU lookup, and batch
shopping-list pricing. Each task runs the MCP server and a deterministic local
stand-in for Trader Joe's two public website endpoints inside one task-local
...
review. Uploaded runs are named `ci-evals-trader-joes`.
