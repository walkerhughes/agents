# Safeway agentic evals

Four Harbor tasks cover nearest-store selection, store-scoped product search,
product-ID refresh, and shopping-list pricing. Each task runs the MCP server and a
deterministic local stand-in for Safeway's public store resolver, address, and search
endpoints inside one task-local environment. That layout runs with both Harbor's
Docker and Modal providers.
...
review. Uploaded runs are named `ci-evals-safeway`.
